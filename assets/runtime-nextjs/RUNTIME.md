...
`ALLOW_UNSAFE_LOCAL_PREVIEW=true` enables a passwordless role selector only outside production. Every page and mutation still enforces the generated permission matrix on the server. Production ignores this local path and fails closed until `src/features/auth/adapter.ts` is connected to a reviewed identity provider and external subjects are provisioned in `app_user`.

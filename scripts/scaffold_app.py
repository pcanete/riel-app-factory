                ...
                if not isinstance(capabilities, list):
                    ...
                    for capability in capabilities:
                        if capability not in CAPABILITIES:
                            errors.append(f"{path}.capabilities has an unknown capability: {capability}.")
                    if len(capabilities) != len(set(capabilities)):
                        errors.append(f"{path}.capabilities contains duplicates.")
        for key in duplicates(role_keys):
            errors.append(f"Duplicate role key: {key}.")
        declared = [role for role in roles if isinstance(role, dict) and "capabilities" in role]
        if declared and len(declared) != len(roles):
            errors.append("Every role must declare capabilities once capabilities are used.")
        if declared and not any("manage_users" in role.get("capabilities", []) for role in declared):
            errors.append("At least one role must declare manage_users once capabilities are used.")
...
def declared_capabilities(spec: dict[str, Any]) -> dict[str, list[str]] | None:
    if not any("capabilities" in role for role in spec["roles"]):
        return None
    return {
        role["key"]: sorted(set(role.get("capabilities", []))) for role in spec["roles"]
    }
...
def compile_permissions(spec: dict[str, Any]) -> str:
    ...
    literal = "null" if capabilities is None else typescript_literal(capabilities)
    return (
        ...
        "/* Administrative capabilities per role. `null` preserves the legacy fallback. */\n"
        "export const generatedCapabilities: Record<string, readonly string[]> | null =\n"
        f"  {literal};\n"
    )
...
def compile_report(spec: dict[str, Any]) -> str:
    ...
    if capabilities is None:
        ...
        capability_gate = "- Declare administrative `capabilities` explicitly on every role before production."
    else:
        administrators = sorted(key for key, granted in capabilities.items() if "manage_users" in granted)
        capability_note = f"- Administrative capabilities are explicit; user management is limited to: {', '.join(administrators)}."
        capability_gate = "- Confirm the declared user, settings, agent, audit, and rules capabilities with the client."
    lines = [

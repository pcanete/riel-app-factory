...
class ScaffoldTests(unittest.TestCase):
    ...
    def test_scaffold_is_complete_in_new_directory(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue((output / "src/platform/mcp/server.ts").is_file())
            ...
            database_runtime = (output / "src/lib/db.ts").read_text(encoding="utf-8")
            ...
            mcp_server = (output / "src/platform/mcp/server.ts").read_text(encoding="utf-8")
            record_operations = (output / "src/app/record-operations/actions.ts").read_text(encoding="utf-8")
            import_actions = (output / "src/app/records/[entity]/import/actions.ts").read_text(encoding="utf-8")
            self.assertEqual(actions.count("revalidateAfterWrite("), 3)
            self.assertEqual(mcp_server.count("revalidateAfterWrite("), 3)
            self.assertEqual(record_operations.count("revalidateAfterWrite("), 3)
            self.assertEqual(import_actions.count("revalidateAfterWrite("), 1)
